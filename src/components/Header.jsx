    ...
    <header className="sticky top-0 z-[900] h-16 border-b border-slate-200/80 bg-white/95 backdrop-blur-xl dark:border-white/10 dark:bg-[#0b1712]/95 sm:h-18">
      ...
      {mobileOpen && <div className="absolute inset-x-0 top-full border-b border-slate-200 bg-white p-4 shadow-xl dark:border-white/10 dark:bg-[#0b1712] lg:hidden">
        ...
        <div className="mt-3 flex items-center justify-between">
          <div className="flex rounded-xl bg-slate-100 p-1 dark:bg-white/5">{['en', 'ru', 'uz'].map((language) => <button key={language} onClick={() => changeLanguage(language)} className={`min-h-10 rounded-lg px-3 text-xs font-bold uppercase ${i18n.language === language ? 'bg-white shadow dark:bg-white/10' : ''}`}>{language}</button>)}</div>
          <button onClick={() => setDark((value) => !value)} className="icon-button">{dark ? <Sun size={18} /> : <Moon size={18} />}</button>
        </div>
        <div className="mt-3 border-t border-slate-200 pt-3 dark:border-white/10">{user ? <div className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-xl bg-emerald-50 text-emerald-600 dark:bg-emerald-400/10"><UserRound size={18} /></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-extrabold">{user.name}</p><p className="truncate text-[11px] text-slate-400">{user.email}</p></div><button onClick={() => { logout(); setMobileOpen(false) }} className="icon-button text-rose-500"><LogOut size={17} /></button></div> : <Link to="/register" onClick={() => setMobileOpen(false)} className="secondary-button w-full justify-center"><UserRound size={17} />Регистрация</Link>}</div>
      </div>}
    </header>
  )
}
