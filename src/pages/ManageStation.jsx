...
export function ManageStation() {
  ...
  const deleteMutation = useMutation({ mutationFn: stationApi.remove, onSuccess: (id) => { removeStation(id); queryClient.invalidateQueries({ queryKey: ['stations'] }); toast('Station deleted') }, onError: () => toast('Could not delete station', 'error') })
