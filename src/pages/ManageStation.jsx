...
export function ManageStation() {
  ...
  return (
    <div className="page-shell">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">Network operations</p><h1 className="mt-2 text-2xl font-black tracking-tight sm:text-4xl">Charging station manager</h1><p className="mt-2 text-sm text-slate-500">Add and maintain public EV infrastructure across Tashkent.</p></div><button onClick={() => setFormOpen((value) => !value)} className="primary-button w-full justify-center sm:w-auto">{formOpen ? <X size={17} /> : <Plus size={17} />}{formOpen ? 'Close form' : 'Add station'}</button></div>
      ...
      <section className="mt-8"><div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-black">All stations</h2><span className="rounded-full bg-slate-200 px-3 py-1 text-xs font-bold dark:bg-white/10">{stations.length} total</span></div>
        <div className="panel overflow-hidden"><div className="hidden grid-cols-[1.5fr_1fr_1fr_1fr_auto] gap-4 border-b border-slate-200 bg-slate-50 px-5 py-3 text-[10px] font-extrabold uppercase tracking-wider text-slate-400 dark:border-white/10 dark:bg-white/[.03] md:grid"><span>Station</span><span>Network</span><span>Connector</span><span>Status</span><span>Actions</span></div>
          {isLoading ? <div className="h-40 animate-pulse bg-slate-100 dark:bg-white/5" /> : stations.map((station) => <div key={station.id} className="grid grid-cols-[1fr_auto] items-center gap-3 border-b border-slate-100 px-4 py-4 last:border-0 dark:border-white/5 md:grid-cols-[1.5fr_1fr_1fr_1fr_auto] md:gap-4 md:px-5"><div className="flex min-w-0 items-center gap-3"><div className="grid size-9 shrink-0 place-items-center rounded-xl bg-emerald-50 text-emerald-600 dark:bg-emerald-400/10"><BatteryCharging size={17} /></div><div className="min-w-0"><p className="truncate text-sm font-extrabold">{station.name}</p><p className="mt-0.5 flex items-center gap-1 text-xs text-slate-500"><MapPin size={11} /> {station.district}</p></div></div><span className="hidden text-xs font-bold text-slate-500 md:block">{station.network}</span><span className="hidden text-xs font-bold md:block">{station.connectors.join(', ')}</span><div className="hidden md:block"><StatusBadge status={station.status} compact /></div><div className="flex gap-1"><a href={`/stations/${station.id}`} className="icon-button" aria-label="Edit station"><Edit3 size={15} /></a><button disabled={deleteMutation.isPending} onClick={() => setDeletingStation(station)} className="icon-button hover:text-rose-500" aria-label="Delete station"><Trash2 size={15} /></button></div></div>)}</div>
      </section>
